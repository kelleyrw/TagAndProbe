...
pushd $TNP/tools
root_cmd="makeCMS2ClassFiles.C+(\"$file_name\", false, \"leptons\", \"\", \"LeptonTree\", \"lepton_tree\", \"lepton_tree_obj\")"
echo "[makeLeptonTreeClassFiles.sh] calling"
echo $root_cmd
root -b -l -q "$root_cmd" 

# move LeptonTree.h
mv LeptonTree.h $TNP/interface/.

# move LeptonTree.cc and change include path for LeptonTree.h to conform with SCRAM convention
sed 's/\#include "LeptonTree.h"/\#include "TagAndProbe\/Analysis\/interface\/LeptonTree.h"/g' LeptonTree.cc > $TNP/src/LeptonTree.cc
rm LeptonTree.cc
